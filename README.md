...
> [!IMPORTANT]
> Officially, the Operator only supports OpenAI and BAM as large language model (LLM) providers, since we do not test for other models, but technically, if you have an OpenAI API compatible model server (Ollama, VLLM, MLX), it should work.
