...
37. The container mounts the TLS Secret (`openshift-mcp-server-tls`, generated by service-ca via the Service annotation) as a volume. The `tls.crt` and `tls.key` files from this Secret are used by `--tls-cert` and `--tls-key`.
37a. The container mounts a TOML configuration file from a ConfigMap (`openshift-mcp-server-config`). The TOML includes TLS cert/key paths, denied-resources list, and enabled toolsets.
