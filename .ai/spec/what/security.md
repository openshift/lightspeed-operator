...
11. Standard providers must have a secret with the `apitoken` key. Azure OpenAI providers must have either `apitoken` or all three of `client_id`, `tenant_id`, `client_secret`. Generic providers use the `credentialKey` field (defaults to `apitoken`). LlamaStackGeneric providers may omit credentials entirely for public endpoints.
12. Custom TLS secrets are validated via `ValidateTLSSecret()` to ensure they contain `tls.crt` and `tls.key`.
13. In AppServer mode, provider credentials are mounted as read-only volume files at `/etc/apikeys/<secretName>/`, never as environment variables. In LCore mode, credentials are set as environment variables via `secretKeyRef` references (e.g., `<PROVIDER_NAME>_API_KEY` from the secret key).
...
1. The operator must not store credentials in ConfigMaps or plain environment variables directly. In AppServer mode, secrets are file-mounted. In LCore mode, Kubernetes `secretKeyRef` env var references are acceptable.
