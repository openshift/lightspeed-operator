...
### Prometheus Metrics
1. The operator creates ServiceMonitor resources for the operator itself (`controller-manager-metrics-monitor`), the backend (`lightspeed-app-server-monitor`), and the OTEL Collector (`lightspeed-otel-collector-monitor`) if Prometheus Operator CRDs are available. Availability is checked at startup via `IsPrometheusOperatorAvailable()`.
