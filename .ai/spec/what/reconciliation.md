...
9. Phase 1 component groups, in execution order: chat Console UI, PostgreSQL, OpenShift MCP server (when introspection enabled), RHOKP (when OKP enabled, i.e. `!byokRAGOnly`), agentic console plugin (when the agentic console image is set), agentic alerts adapter (when `configMapRef` is set), OTEL Collector, and the application server. Because Phase 1 is continue-on-error over independent resources, this ordering is not a dependency contract; Phase 2 ordering (rule 12) is where sequencing matters.
...
12c. OTEL Collector Phase 2 (OLS-3510 / OLS-3656): Service (OTLP gRPC `:4317`, HTTP `:4318`, admin HTTPS `:8080`, metrics HTTPS `:8888`, serving-cert annotation), TLS secret wait (`lightspeed-otel-collector-cert`), Deployment (1 replica, metrics container port), ServiceMonitor (`lightspeed-otel-collector-monitor`, skipped if Prometheus CRDs unavailable). Reconciled after the app-server in Phase 2 (app-server connects to the collector by Service DNS, so Service pre-existence is not required). Agentic OTEL connectivity is published via appserver client CA Secrets + `agenticintegration` handoff ConfigMap (not a collector client ConfigMap).
12d. Appserver Phase 2 creates/refreshes client-only CA Secrets (`lightspeed-agentic-otel-ca`, `lightspeed-agentic-mcp-ca` when introspection enabled, and `lightspeed-agentic-rhokp-ca` when OKP enabled) from service-ca source ConfigMaps via table-driven `RefreshClientCASecrets`, mounts them into the app-server Deployment, then reconciles Service/TLS/Deployment.
