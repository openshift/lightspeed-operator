...
The `ols-fbc-v5-0` Konflux Application must invoke `update-catalog` with
`bundle-variant: v2`. The catalog generator then reads the
`lightspeed-agentic-operator-bundle` component (the v2 bundle) from the
release snapshot. Its rendered bundle retains the v2
`com.redhat.openshift.versions: >=v5.0` annotation in the OCP 5.0 fragment.
