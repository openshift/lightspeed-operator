...
var _ = Describe("Reconciliation From OLSConfig CR", Ordered, func() {
	...
	It("should remove LLM secret annotation and propagate config when credentialHotReload is enabled", func() {
		...
		By("capture deployment generation before enabling hot-reload")
		deployment := &appsv1.Deployment{
			ObjectMeta: metav1.ObjectMeta{
				Name:      AppServerDeploymentName,
				Namespace: OLSNameSpace,
		...
		err = client.Get(deployment)
		Expect(err).NotTo(HaveOccurred())
		genBeforeHotReload := deployment.Generation
		...
		By("wait for deployment controller to process the config change (no full rollout needed)")
		err = client.WaitForDeploymentNextGeneration(deployment, genBeforeHotReload)
		Expect(err).NotTo(HaveOccurred())
		...
		By("wait for deployment controller to process the config change (no full rollout needed)")
		err = client.WaitForDeploymentNextGeneration(deployment, generationBeforeSecretUpdate)
		Expect(err).NotTo(HaveOccurred())
