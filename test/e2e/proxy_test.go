...
var _ = Describe("Proxy test", Ordered, Label("Proxy"), FlakeAttempts(5), func() {
	...
	setupProxyConfig := func(proxyURL string, proxyCACertRef *corev1.ConfigMapKeySelector) {
	...
	It("should be able to query the application server with https proxy", func() {
		setupProxyConfig("https://"+squidHostname+":"+strconv.Itoa(httpsPort), &corev1.ConfigMapKeySelector{
			LocalObjectReference: corev1.LocalObjectReference{
				Name: "proxy-ca",
			},
		})
