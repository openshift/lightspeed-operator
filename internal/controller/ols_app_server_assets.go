...
func generateMCPServerConfigs(cr *olsv1alpha1.OLSConfig) []MCPServerConfig {
	...
	for _, server := range cr.Spec.MCPServers {
		servers = append(servers, MCPServerConfig{
			...
			SSE:            generateMCPStreamableHTTPTransportConfig(&server, SSE),
			StreamableHTTP: generateMCPStreamableHTTPTransportConfig(&server, StreamableHTTP),
		})
	}
	return servers
}

func generateMCPStreamableHTTPTransportConfig(server *olsv1alpha1.MCPServer, mcpType MCPTransport) *StreamableHTTPTransportConfig {
	if server == nil || server.Transport != string(mcpType) {
		return nil
	}
