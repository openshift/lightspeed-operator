...
func restartDeployment(r reconciler.Reconciler, ctx context.Context, affectedDeployments []string, namespace string, name string, useLCore bool) {
	...
	if usesPostgresCache && len(postgresDeployments) > 0 && len(appServerDeployments) > 0 {
		...
		// Step 3 & 4: Wait for Postgres and restart app server in background
		// Use a background goroutine to avoid blocking the event handler
		go func() {
			// Create a new background context with timeout to avoid using the event handler's context
			backgroundCtx, cancel := context.WithTimeout(context.Background(), 6*time.Minute)
			defer cancel()

			r.GetLogger().Info("waiting for PostgreSQL deployment to be ready before restarting app server",
				"timeout", "5 minutes")
			err := waitForDeploymentReady(r, backgroundCtx, utils.PostgresDeploymentName, 5*time.Minute)
			if err != nil {
			...
			// Restart app server after postgres is ready (or timeout)
			for _, depName := range appServerDeployments {
				...
				restartSingleDeployment(r, backgroundCtx, depName, name, namespace)
			}
		}()
	...
	if usesPostgresCache && len(postgresDeployments) > 0 && len(appServerDeployments) == 0 {
		...
		// Wait for postgres and restart app server in background
		go func() {
			...
			if err != nil {
				...
				// Restart app server
				if useLCore {
					restartSingleDeployment(r, backgroundCtx, utils.LCoreDeploymentName, name, namespace)
				} else {
					restartSingleDeployment(r, backgroundCtx, utils.OLSAppServerDeploymentName, name, namespace)
				}
			}
		}()
	}
