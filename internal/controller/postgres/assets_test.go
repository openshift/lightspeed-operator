...
var _ = Describe("App postgres server assets", func() {
	...
	validatePostgresDeployment := func(dep *appsv1.Deployment, password string, with_pvc bool) {
		...
		Expect(dep.Spec.Template.Spec.Containers[0].Image).To(Equal(postgresImage))
	...
	Context("complete custom resource", func() {
		BeforeEach(func() {
			postgresImage = "lightspeed-service-postgres:latest"
			// Update the test reconciler's postgres image
			if tr, ok := testReconcilerInstance.(*testReconciler); ok {
				tr.postgresImage = postgresImage
			}
