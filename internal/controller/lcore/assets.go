...
func buildLlamaStackInferenceProviders(_ reconciler.Reconciler, cr *olsv1alpha1.OLSConfig) ([]interface{}, error) {
	...
	for _, provider := range cr.Spec.LLMConfig.Providers {
		...
		case "azure_openai":
			...
			if provider.URL != "" {
				// Llama Stack's AzureConfig Pydantic model requires 'api_base'
				// Note: This triggers a bug in LiteLLM where it adds unsupported 'api_type' parameter
				// See: https://github.com/BerriAI/litellm/issues/6241
		...
		case "watsonx", "rhoai_vllm", "rhelai_vllm", "bam":
			...
			return nil, fmt.Errorf("provider type '%s' (provider '%s') is not cyrrentl supported by Llama Stack. Supported types: openai, azure_openai", provider.Type, provider.Name)
