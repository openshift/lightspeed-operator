...
func buildLlamaStackInferenceProviders(_ reconciler.Reconciler, _ context.Context, cr *olsv1alpha1.OLSConfig) ([]interface{}, error) {
	...
	for _, provider := range cr.Spec.LLMConfig.Providers {
		...
		if !hasCredentialField(configCopy) {
			...
			// Predefined provider types: map to Llama Stack provider types
			...
			case "generic":
				// This case should not be reached due to CEL validation
				// but include it for defensive programming
				return nil, fmt.Errorf("provider type 'generic' (provider '%s') requires providerType and config fields to be set", provider.Name)

			default:
				// Unknown provider type
				return nil, fmt.Errorf("unknown provider type '%s' (provider '%s'). Supported types: openai, azure_openai, rhoai_vllm, rhelai_vllm, generic", provider.Type, provider.Name)
			}
		}
