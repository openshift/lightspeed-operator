...
# You can use it as an arg. (E.g make bundle BUNDLE_VARIANT=v1 BUNDLE_TAG=1.1.4)
BUNDLE_TAG ?= $(if $(filter v2,$(BUNDLE_VARIANT)),2.0.0,1.1.4)
...
test: manifests generate fmt vet envtest test-crds ## Run local tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" go test -v ./internal/... -coverprofile cover.out -p 6 -timeout 10m
...
$(OCP_IMAGESTREAM_CRD_FILE): $(TEST_CRD_DIR)
	cp $(OCP_IMAGESTREAM_CRD_URL) $(OCP_IMAGESTREAM_CRD_FILE)
