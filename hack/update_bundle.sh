...
DEPLOYMENT_PATCH_BACKUP="${TEMP_BUNDLE_CONTAINER_FILE}.deployment-patch"
...
cleanup() {
  # The generator consumes a variant-specific patch/template but must leave the
  # source tree unchanged, including after a failed generation.
  if [ -f "${DEPLOYMENT_PATCH_BACKUP}" ]; then
    mv "${DEPLOYMENT_PATCH_BACKUP}" "${DEPLOYMENT_PATCH_FILE}"
  fi
...
if [ "${CSV_TEMPLATE}" != "${CSV_BASE_TEMPLATE}" ] && [ -f "${CSV_TEMPLATE}" ]; then
