...
# operator-sdk generate bundle overwrites the tracked legacy Dockerfile even
# when writing a variant to bundle-v1/ or bundle-v2/.
LEGACY_DOCKERFILE_BACKUP="${TEMP_BUNDLE_CONTAINER_FILE}.dockerfile"
cp bundle.Dockerfile "${LEGACY_DOCKERFILE_BACKUP}"

cleanup() {
  cp "${DEPLOYMENT_PATCH_BACKUP}" "${DEPLOYMENT_PATCH_FILE}"
  mv "${LEGACY_DOCKERFILE_BACKUP}" bundle.Dockerfile
...
elif [ "${BUNDLE_VARIANT}" = "v1" ]; then
  echo "missing classic v1 CSV template: ${CSV_TEMPLATE}" >&2
  exit 1
fi
...
sed -i -e "s|COPY bundle/manifests|COPY ${BUNDLE_DIR}/manifests|" \
       ...
       -e "s|^LABEL release=.*|LABEL release=${BUNDLE_VERSION}|" \
       "${BUNDLE_DOCKERFILE}"
...
# argument of the classic controller. Substitute its image directly.
if [ "${BUNDLE_VARIANT}" = "v2" ]; then
...
# The existing v1 release and CI pipelines build from the tracked bundle/ path.
# Keep it in sync with the generated classic bundle until those pipelines move
# to the versioned artifact. Never replace it when generating v2.
if [ "${BUNDLE_VARIANT}" = "v1" ]; then
  rm -rf ./bundle
  cp -a "./${BUNDLE_DIR}" ./bundle
fi
