...
JAEGER_IMAGE="quay.io/jaegertracing/jaeger:2.21.0"
...
if [[ "$DELETE" == true ]]; then
  ...
  oc delete secret jaeger-otlp-tls -n "${NAMESPACE}" --ignore-not-found
...
# Wait for the service-ca to generate and populate the TLS secret
echo "Waiting for service-ca to issue TLS certificate..."
for i in $(seq 1 30); do
  cert_length=$(oc get secret jaeger-otlp-tls -n "${NAMESPACE}" -o jsonpath='{.data.tls\.crt}' 2>/dev/null | wc -c || true)
  key_length=$(oc get secret jaeger-otlp-tls -n "${NAMESPACE}" -o jsonpath='{.data.tls\.key}' 2>/dev/null | wc -c || true)
  if [[ "$cert_length" -gt 0 && "$key_length" -gt 0 ]]; then
